...
def solve_implicit(ks, a, b, c, d, b_edge=None, d_edge=None):
    ...
    edge_mask = land_mask & (cp.arange(a.shape[2])[np.newaxis, np.newaxis, :]
                             == ks[:, :, np.newaxis])
    water_mask = land_mask & (cp.arange(a.shape[2])[np.newaxis, np.newaxis, :]
                              >= ks[:, :, np.newaxis])

    a_tri = water_mask * a * cp.logical_not(edge_mask)
...
def _calc_cr(rjp, rj, rjm, vel):
    ...
    return where(vel > 0., rjm, rjp) / where(cp.abs(rj) < eps, eps, rj)


def pad_z_edges(arr):
    arr_shape = list(arr.shape)
    arr_shape[2] += 2
    out = cp.zeros(arr_shape, arr.dtype)
    out[:, :, 1:-1] = arr
    return out


def limiter(cr):
    return cp.maximum(0., cp.maximum(cp.minimum(1., 2 * cr), cp.minimum(2., cr)))
...
def _adv_superbee(vel, var, mask, dx, axis, cost, cosu, dt_tracer):
    ...
    uCFL = cp.abs(velfac * vel[s] * dt_tracer / dx)
    rjp = (var[sp2] - var[sp1]) * mask[sp1]
    rj = (var[sp1] - var[s]) * mask[s]
    rjm = (var[s] - var[sm1]) * mask[sm1]
    cr = limiter(_calc_cr(rjp, rj, rjm, vel[s]))
    return velfac * vel[s] * (var[sp1] + var[s]) * 0.5 - cp.abs(velfac * vel[s]) * ((1. - cr) + uCFL * cr) * rj * 0.5
...
def integrate_tke(u, v, w, maskU, maskV, maskW, dxt, dxu, dyt, dyu, dzt, dzw, cost, cosu, kbot, kappaM, mxl, forc, forc_tke_surface, tke, dtke):
    ...
    flux_east = cp.zeros_like(maskU)
    flux_north = cp.zeros_like(maskU)
    flux_top = cp.zeros_like(maskU)

    sqrttke = cp.sqrt(cp.maximum(0., tke[:, :, :, tau]))
    ...
    a_tri = cp.zeros_like(maskU[2:-2, 2:-2])
    b_tri = cp.zeros_like(maskU[2:-2, 2:-2])
    c_tri = cp.zeros_like(maskU[2:-2, 2:-2])
    d_tri = cp.zeros_like(maskU[2:-2, 2:-2])
    delta = cp.zeros_like(maskU[2:-2, 2:-2])
