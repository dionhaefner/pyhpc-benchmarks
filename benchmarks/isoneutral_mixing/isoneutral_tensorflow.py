...
def get_drhodS(salt, temp, p):
    betaS = 0.78e-3
    rho0 = 1024.
    return betaS * rho0 * tf.ones_like(temp)
...
def isoneutral_diffusion_pre(maskT, maskU, maskV, maskW, dxt, dxu, dyt, dyu, dzt, dzw, cost, cosu, salt, temp, zt, K_iso, K_11, K_22, K_33, Ai_ez, Ai_nz, Ai_bx, Ai_by):
    ...
    dTdx = tf.zeros_like(K_11)
    dSdx = tf.zeros_like(K_11)
    dTdy = tf.zeros_like(K_11)
    dSdy = tf.zeros_like(K_11)
    dTdz = tf.zeros_like(K_11)
    dSdz = tf.zeros_like(K_11)

    """
    drho_dt and drho_ds at centers of T cells
    """
    drdT = maskT * get_drhodT(
        salt[:, :, :, tau], temp[:, :, :, tau], tf.math.abs(zt)
    )
    ...
    def dm_taper(sx):
        ...
        """
        return 0.5 * (1. + np.tanh((-tf.math.abs(sx) + iso_slopec) / iso_dslope))
...
isoneutral_diffusion_pre_tf = tf.function(isoneutral_diffusion_pre)


def run(*inputs):
    inputs = (tf.convert_to_tensor(a) for a in inputs)
    return isoneutral_diffusion_pre_tf(*inputs)[-1]
