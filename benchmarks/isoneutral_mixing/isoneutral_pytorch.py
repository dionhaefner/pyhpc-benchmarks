...
def isoneutral_diffusion_pre(
    ...
    for kr in range(2):
        ...
        for ip in range(2):
            ...
            sxe = -drodxe / (torch.min(drodze, torch.tensor([0.0])) - epsln)
            taper = dm_taper(sxe)
            sumz[:, :, ki:] += (
                dzw[None, None, :su]
                * maskU[1:-2, 2:-2, ki:]
                * torch.max(
                    torch.tensor([K_iso_steep]), diffloc[1:-2, 2:-2, ki:] * taper
                )
            )
    ...
    for kr in range(2):
        ...
        for jp in range(2):
            ...
            syn = -drodyn / (torch.min(torch.tensor([0.0]), drodzn) - epsln)
            taper = dm_taper(syn)
            sumz[:, :, ki:] += (
                dzw[None, None, :su]
                * maskV[2:-2, 1:-2, ki:]
                * torch.max(
                    torch.tensor([K_iso_steep]), diffloc[2:-2, 1:-2, ki:] * taper
                )
            )
    ...
    for kr in range(2):
        ...
        for ip in range(2):
            ...
            sxb = -drodxb / (torch.min(torch.tensor([0.0]), drodzb) - epsln)
            taper = dm_taper(sxb)
            sumx += (
        ...
        for jp in range(2):
            ...
            syb = -drodyb / (torch.min(torch.tensor([0.0]), drodzb) - epsln)
            taper = dm_taper(syb)
            sumy += (
                facty[None, :, None]
                * K_iso[2:-2, 2:-2, :-1]
...
@torch.jit.script
def isoneutral_diffusion_pre_gpu(
    maskT,
    maskU,
    maskV,
    maskW,
    ...
    for kr in range(2):
        ...
        for ip in range(2):
            ...
            sxe = -drodxe / (
                torch.min(drodze, torch.tensor([0.0], device="cuda")) - epsln
            )
            taper = dm_taper(sxe)
            sumz[:, :, ki:] += (
                dzw[None, None, :su]
                * maskU[1:-2, 2:-2, ki:]
                * torch.max(
                    torch.tensor([K_iso_steep], device="cuda"),
    ...
    for kr in range(2):
        ...
        for jp in range(2):
            ...
            syn = -drodyn / (
                torch.min(torch.tensor([0.0], device="cuda"), drodzn) - epsln
            )
            taper = dm_taper(syn)
            sumz[:, :, ki:] += (
                dzw[None, None, :su]
    ...
    for kr in range(2):
        ...
        for ip in range(2):
            ...
            sxb = -drodxb / (
                torch.min(torch.tensor([0.0], device="cuda"), drodzb) - epsln
            )
            taper = dm_taper(sxb)
            sumx += (
                dxu[1 + ip : -3 + ip, None, None]
