...
@ti.kernel
def gsw_dHdT(sa_arr: ti.template(), ct_arr: ti.template(), p_arr: ti.template(), out: ti.template()):
    """
    ...
    p      : sea pressure                                    [dbar]
    """
    for i, j, k in ti.ndrange(*out.shape):
        sa = sa_arr[i, j, k]
        ct = ct_arr[i, j, k]
        p = p_arr[0, 0, k]
...
def prepare_inputs(sa, ct, p, device="cpu"):
    sa_field, ct_field, p_field, out_field = get_fields((sa.shape, ct.shape, p.shape), num_scratch=1)
    sa_field.from_numpy(sa)
    ct_field.from_numpy(ct)
    p_field.from_numpy(p)
    out_field.fill(0)
    return sa_field, ct_field, p_field, out_field
